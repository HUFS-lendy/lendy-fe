...
const stages = [
  ...
  { key: "waiting", title: "대기열 입장", description: "대기 순번과 예상 대기 시간이 표시되는 화면입니다." },
  { key: "selection", title: "기자재 선택", description: "대기열 통과 후 실제 기자재를 선택하는 화면입니다." },
      ...
      <div className="mx-auto max-w-6xl">
        <div className="flex items-end justify-between gap-8 border-b border-white/10 pb-8">
          <div>
            ...
            <p className="mt-3 max-w-2xl text-sm leading-6 text-neutral-400">실제 대기열, 예약 데이터와 재고를 변경하지 않고 학생이 보는 예약 전 과정을 단계별로 확인합니다.</p>
          </div>
          <button onClick={() => open("countdown")} className="flex shrink-0 items-center gap-2 bg-white px-5 py-2.5 text-sm font-semibold text-black hover:bg-neutral-200">
            <MonitorPlay className="h-4 w-4" /> 전체 화면 열기
          </button>
        </div>
